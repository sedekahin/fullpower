...
<form method="post" id="wpr_new">
	...
	<p class="submit" style="float:right;margin:0;padding: 10px 0;">
		...
		<!--Tooltip--><a class="tooltip" href="#">?<span><?php _e('Distribute the chance of being used between all templates currently added to this campaign. This can especially be helpful after adding new templates via the buttons to the left.',"wprobot") ?></span></a>
	</p>
	...
	<table class="addt" width="100%">
		...
		<tr>
			<td width="50%">
				...
				<select name="amazon_department">
					...
					<?php if($ll=="com") {?><option value="Grocery" <?php if($_POST['amazon_department'] == "Grocery") {echo "selected";}?>><?php _e("Grocery","wprobot") ?></option><?php } ?>
					...
					<?php if($ll=="com") {?><option value="KindleStore" <?php if($_POST['amazon_department'] == "KindleStore") {echo "selected";}?>><?php _e("Kindle Store","wprobot") ?></option><?php } ?>
					...
					<?php if($ll=="com") {?><option value="MusicalInstruments" <?php if($_POST['amazon_department'] == "MusicalInstruments") {echo "selected";}?>><?php _e("Musical Instruments","wprobot") ?></option><?php } ?>
		...
		<?php if(!empty($options["wpr_rewrite_email"]) && !empty($options["wpr_rewrite_key"]) && $options["wpr_rewrite_active"] == "Yes") { ?>		
		<tr>
			<td width="100%" colspan="2">
			<b><?php _e("Rewriter","wprobot") ?></b><br/>
			<input type="checkbox" name="wpr_rewriter" value="1" <?php if($_POST["wpr_rewriter"] == 1) {echo "checked";} ?>/> <?php _e("Activate UCG Rewriter for this campaign.","wprobot") ?>			
			</td>
		</tr>		
		<?php } ?>	
